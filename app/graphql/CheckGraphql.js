...
{  
  ...
  context:{  //这个参数是node 上下文 可以传递request，response，next等
    ctx: {
      request: {},
      response: {},
    },
...
class CheckGraphql {
  ...
  static async init(options) {
    ...
    // 验证服务户端Schema
    await this.validateSeverSchema(this.options);
    // 验证客户端Schema
    await this.validateClientSchema();
    // 一起验证客户端服务端Schema
    await this.validateSeverClientSchema();
  ...
  // 验证 SeverSchema
  static validateSeverSchema = async (options) => {
    let {
    ...
    // console.log('directiveResolvers=',directiveResolvers)
    ...
    try {
      ...
      const typeDefs = schemas.length
        ? [serverRootSchema, schemas].map((item) => gql(item))
        : [gql(serverSchema)];
      // 验证 SeverSchema
  ...
  static validateClientSchema = async () => {
    let {
  ...
  static validateSeverClientSchema = async () => {
    let {
      serverSchema: { schema: serverSchema = "", resolvers = {} } = {},
      clientSchema: { schema: clientSchema = "", variables = {} } = {},
    } = this.options;
